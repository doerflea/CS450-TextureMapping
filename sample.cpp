...
void InitTextures(){
   ...
   glGenTextures(1, &cats);

    unsigned char* Texture = BmpToTexture((char*) "cats1.bmp", &width, &height);

    glBindTexture(GL_TEXTURE_2D, cats);
...
Display( )
{
   ...
   glLoadIdentity( );
   glColor3f( 1., 1., 1. );
   DoRasterString( 5., 5., 0., (char *)"Anneliese Doerfler - My Handsome Boy" );
...
MjbSphere( float radius, int slices, int stacks )
{
	...
	for( int ilat = 0; ilat < NumLats; ilat++ ){
	   ...
	   for( int ilng = 0; ilng < NumLngs; ilng++ ){
		...
		p->nz = z;
		if( DistortOn ){
		   p->s = (lng + M_PI ) / ( 2.*M_PI ) + y * .20 * Time;
		   p->t = (.1 * x * Time +lat + M_PI/2. ) / M_PI;
		}
		else{
		   p->s = ( lng + M_PI    ) / ( 2.*M_PI );
		   p->t = ( lat + M_PI/2. ) / M_PI;
